 ...
 * Page class
 */
class MessageTemplatesList extends MessageTemplates
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'message_templates';

    // Page object name
    public $PageObjName = "MessageTemplatesList";
    ...
    public $FormName = "fmessage_templateslist";
    ...
    public function __construct()
    {
        ...
        // Table object (message_templates)
        if (!isset($GLOBALS["message_templates"]) || get_class($GLOBALS["message_templates"]) == PROJECT_NAMESPACE . "message_templates") {
            $GLOBALS["message_templates"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "messagetemplatesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "messagetemplatesdelete";
        $this->MultiUpdateUrl = "messagetemplatesupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'message_templates');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("message_templates");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->recepient_group->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->recepient_group);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->recepient_group->AdvancedSearch->toJson(), ","); // Field recepient_group
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmessage_templatessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field recepient_group
        $this->recepient_group->AdvancedSearch->SearchValue = @$filter["x_recepient_group"];
        $this->recepient_group->AdvancedSearch->SearchOperator = @$filter["z_recepient_group"];
        $this->recepient_group->AdvancedSearch->SearchCondition = @$filter["v_recepient_group"];
        $this->recepient_group->AdvancedSearch->SearchValue2 = @$filter["y_recepient_group"];
        $this->recepient_group->AdvancedSearch->SearchOperator2 = @$filter["w_recepient_group"];
        $this->recepient_group->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->recepient_group;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->recepient_group); // recepient_group
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmessage_templateslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmessage_templateslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("recepient_group", $this->createColumnOption("recepient_group"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmessage_templatessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmessage_templatessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmessage_templateslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->recepient_group->setDbValue($row['recepient_group']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // recepient_group
            if (strval($this->recepient_group->CurrentValue) != "") {
                $this->recepient_group->ViewValue = $this->recepient_group->optionCaption($this->recepient_group->CurrentValue);
            } else {
                $this->recepient_group->ViewValue = null;
            }
            $this->recepient_group->ViewCustomAttributes = "";
            ...
            // recepient_group
            $this->recepient_group->LinkCustomAttributes = "";
            $this->recepient_group->HrefValue = "";
            $this->recepient_group->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmessage_templateslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fmessage_templatessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
