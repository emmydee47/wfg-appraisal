 ...
 * Page class
 */
class MainPaScoreList extends MainPaScore
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MainPaScoreList";
    ...
    public $FormName = "fmain_pa_scorelist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "mainpascoreadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mainpascoredelete";
        $this->MultiUpdateUrl = "mainpascoreupdate";
    ...
    public function run()
    {
        ...
        $this->employee_response->Visible = false;
        $this->line_manager_one->setVisibility();
        $this->line_manager_one_response->Visible = false;
        $this->line_manager_two->setVisibility();
        $this->line_manager_two_response->Visible = false;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_appraisal") && $CurrentForm->hasValue("o_appraisal") && $this->appraisal->CurrentValue != $this->appraisal->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_employee") && $CurrentForm->hasValue("o_employee") && $this->employee->CurrentValue != $this->employee->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_line_manager_one") && $CurrentForm->hasValue("o_line_manager_one") && $this->line_manager_one->CurrentValue != $this->line_manager_one->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_line_manager_two") && $CurrentForm->hasValue("o_line_manager_two") && $this->line_manager_two->CurrentValue != $this->line_manager_two->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_consolidate_score") && $CurrentForm->hasValue("o_consolidate_score") && $this->consolidate_score->CurrentValue != $this->consolidate_score->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->appraisal->clearErrorMessage();
        $this->employee->clearErrorMessage();
        $this->line_manager_one->clearErrorMessage();
        $this->line_manager_two->clearErrorMessage();
        $this->consolidate_score->clearErrorMessage();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->appraisal->AdvancedSearch->toJson(), ","); // Field appraisal
        $filterList = Concat($filterList, $this->employee->AdvancedSearch->toJson(), ","); // Field employee
        $filterList = Concat($filterList, $this->employee_response->AdvancedSearch->toJson(), ","); // Field employee_response
        $filterList = Concat($filterList, $this->line_manager_one->AdvancedSearch->toJson(), ","); // Field line_manager_one
        $filterList = Concat($filterList, $this->line_manager_one_response->AdvancedSearch->toJson(), ","); // Field line_manager_one_response
        $filterList = Concat($filterList, $this->line_manager_two->AdvancedSearch->toJson(), ","); // Field line_manager_two
        $filterList = Concat($filterList, $this->line_manager_two_response->AdvancedSearch->toJson(), ","); // Field line_manager_two_response
        $filterList = Concat($filterList, $this->consolidate_score->AdvancedSearch->toJson(), ","); // Field consolidate_score
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmain_pa_scoresrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field appraisal
        $this->appraisal->AdvancedSearch->SearchValue = @$filter["x_appraisal"];
        $this->appraisal->AdvancedSearch->SearchOperator = @$filter["z_appraisal"];
        $this->appraisal->AdvancedSearch->SearchCondition = @$filter["v_appraisal"];
        $this->appraisal->AdvancedSearch->SearchValue2 = @$filter["y_appraisal"];
        $this->appraisal->AdvancedSearch->SearchOperator2 = @$filter["w_appraisal"];
        $this->appraisal->AdvancedSearch->save();
        ...
        // Field employee_response
        $this->employee_response->AdvancedSearch->SearchValue = @$filter["x_employee_response"];
        $this->employee_response->AdvancedSearch->SearchOperator = @$filter["z_employee_response"];
        $this->employee_response->AdvancedSearch->SearchCondition = @$filter["v_employee_response"];
        $this->employee_response->AdvancedSearch->SearchValue2 = @$filter["y_employee_response"];
        $this->employee_response->AdvancedSearch->SearchOperator2 = @$filter["w_employee_response"];
        $this->employee_response->AdvancedSearch->save();

        // Field line_manager_one
        $this->line_manager_one->AdvancedSearch->SearchValue = @$filter["x_line_manager_one"];
        $this->line_manager_one->AdvancedSearch->SearchOperator = @$filter["z_line_manager_one"];
        $this->line_manager_one->AdvancedSearch->SearchCondition = @$filter["v_line_manager_one"];
        $this->line_manager_one->AdvancedSearch->SearchValue2 = @$filter["y_line_manager_one"];
        $this->line_manager_one->AdvancedSearch->SearchOperator2 = @$filter["w_line_manager_one"];
        $this->line_manager_one->AdvancedSearch->save();

        // Field line_manager_one_response
        $this->line_manager_one_response->AdvancedSearch->SearchValue = @$filter["x_line_manager_one_response"];
        $this->line_manager_one_response->AdvancedSearch->SearchOperator = @$filter["z_line_manager_one_response"];
        $this->line_manager_one_response->AdvancedSearch->SearchCondition = @$filter["v_line_manager_one_response"];
        $this->line_manager_one_response->AdvancedSearch->SearchValue2 = @$filter["y_line_manager_one_response"];
        $this->line_manager_one_response->AdvancedSearch->SearchOperator2 = @$filter["w_line_manager_one_response"];
        $this->line_manager_one_response->AdvancedSearch->save();

        // Field line_manager_two
        $this->line_manager_two->AdvancedSearch->SearchValue = @$filter["x_line_manager_two"];
        $this->line_manager_two->AdvancedSearch->SearchOperator = @$filter["z_line_manager_two"];
        $this->line_manager_two->AdvancedSearch->SearchCondition = @$filter["v_line_manager_two"];
        $this->line_manager_two->AdvancedSearch->SearchValue2 = @$filter["y_line_manager_two"];
        $this->line_manager_two->AdvancedSearch->SearchOperator2 = @$filter["w_line_manager_two"];
        $this->line_manager_two->AdvancedSearch->save();

        // Field line_manager_two_response
        $this->line_manager_two_response->AdvancedSearch->SearchValue = @$filter["x_line_manager_two_response"];
        $this->line_manager_two_response->AdvancedSearch->SearchOperator = @$filter["z_line_manager_two_response"];
        $this->line_manager_two_response->AdvancedSearch->SearchCondition = @$filter["v_line_manager_two_response"];
        $this->line_manager_two_response->AdvancedSearch->SearchValue2 = @$filter["y_line_manager_two_response"];
        $this->line_manager_two_response->AdvancedSearch->SearchOperator2 = @$filter["w_line_manager_two_response"];
        $this->line_manager_two_response->AdvancedSearch->save();

        // Field consolidate_score
        $this->consolidate_score->AdvancedSearch->SearchValue = @$filter["x_consolidate_score"];
        $this->consolidate_score->AdvancedSearch->SearchOperator = @$filter["z_consolidate_score"];
        $this->consolidate_score->AdvancedSearch->SearchCondition = @$filter["v_consolidate_score"];
        $this->consolidate_score->AdvancedSearch->SearchValue2 = @$filter["y_consolidate_score"];
        $this->consolidate_score->AdvancedSearch->SearchOperator2 = @$filter["w_consolidate_score"];
        $this->consolidate_score->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->employee_response;
        $searchFlds[] = &$this->line_manager_one_response;
        $searchFlds[] = &$this->line_manager_two_response;
        $searchFlds[] = &$this->consolidate_score;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->appraisal); // appraisal
            $this->updateSort($this->employee); // employee
            $this->updateSort($this->line_manager_one); // line_manager_one
            $this->updateSort($this->line_manager_two); // line_manager_two
            $this->updateSort($this->consolidate_score); // consolidate_score
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->setSessionOrderBy($orderBy);
                $this->id->setSort("");
                $this->appraisal->setSort("");
                $this->employee->setSort("");
                $this->employee_response->setSort("");
                $this->line_manager_one->setSort("");
                $this->line_manager_one_response->setSort("");
                $this->line_manager_two->setSort("");
                $this->line_manager_two_response->setSort("");
                $this->consolidate_score->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_scorelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_scorelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("appraisal", $this->createColumnOption("appraisal"));
            $option->add("employee", $this->createColumnOption("employee"));
            $option->add("line_manager_one", $this->createColumnOption("line_manager_one"));
            $option->add("line_manager_two", $this->createColumnOption("line_manager_two"));
            $option->add("consolidate_score", $this->createColumnOption("consolidate_score"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_pa_scoresrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_pa_scoresrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_pa_scorelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fmain_pa_scorelist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                // Add grid cancel
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_appraisal")) {
            $this->appraisal->setOldValue($CurrentForm->getValue("o_appraisal"));
        }
        ...
        if ($CurrentForm->hasValue("o_employee")) {
        ...
        if ($CurrentForm->hasValue("o_line_manager_one")) {
            $this->line_manager_one->setOldValue($CurrentForm->getValue("o_line_manager_one"));
        }
        ...
        if ($CurrentForm->hasValue("o_line_manager_two")) {
            $this->line_manager_two->setOldValue($CurrentForm->getValue("o_line_manager_two"));
        }
        ...
        if ($CurrentForm->hasValue("o_consolidate_score")) {
            $this->consolidate_score->setOldValue($CurrentForm->getValue("o_consolidate_score"));
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmain_pa_scorelist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fmain_pa_scoresrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
