 ...
 * Page class
 */
class PendingAppraisalRatingsList extends PendingAppraisalRatings
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'pending_appraisal_ratings';

    // Page object name
    public $PageObjName = "PendingAppraisalRatingsList";
    ...
    public $FormName = "fpending_appraisal_ratingslist";
    ...
    public function __construct()
    {
        ...
        // Table object (pending_appraisal_ratings)
        if (!isset($GLOBALS["pending_appraisal_ratings"]) || get_class($GLOBALS["pending_appraisal_ratings"]) == PROJECT_NAMESPACE . "pending_appraisal_ratings") {
            $GLOBALS["pending_appraisal_ratings"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "pendingappraisalratingsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "pendingappraisalratingsdelete";
        $this->MultiUpdateUrl = "pendingappraisalratingsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'pending_appraisal_ratings');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("pending_appraisal_ratings");
                $doc = new $class($tbl);
                $doc->Text = @$content;
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpending_appraisal_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fpending_appraisal_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpending_appraisal_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpending_appraisal_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpending_appraisal_ratingslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->line_manager->ViewValue = FormatNumber($this->line_manager->ViewValue, "");
            $this->line_manager->ViewCustomAttributes = "";

            // level
            $this->level->ViewValue = $this->level->CurrentValue;
            $this->level->ViewValue = FormatNumber($this->level->ViewValue, "");
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fpending_appraisal_ratingslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    ...
    {
         $item = &$this->ListOptions->add("appraise_employee");
    ...
    public function listOptionsRendered()
    {
        ...
        $level = $this->level->CurrentValue;
        $group_id = $this->group_id->CurrentValue;
    		$server = $_SERVER['HTTP_HOST'];
    		$url = "https://".$server."/linemanageroneappraisal/?appraisal_id=".$appraisal_id."&user_id=".$user_id."&level=".$level."&group_id=".$group_id;
            if($server=="localhost")
            {
                $url = "http://".$server."/employeeAppraisal/linemanageroneappraisal/?appraisal_id=".$appraisal_id."&user_id=".$user_id."&level=".$level."&group_id=".$group_id;
            }
            //if($level==2){
    		//	$url = "https://".$server."/linemanagertwoappraisal/?appraisal_id=".$appraisal_id."&user_id=".$user_id."&level=".$level."&group_id=".$group_id;
              //  if($server=="localhost")
                //{
                // $url = "http://".$server."/employeeAppraisal/linemanagertwoappraisal/?appraisal_id=".$appraisal_id."&user_id=".$user_id."&level=".$level."&group_id=".$group_id;
               // }
    		//}
        $this->ListOptions["appraise_employee"]->Body = "<a href='".$url."' class='btn btn-xs btn-primary btn-notify-l1'>Appraise Employee</a>";
    }
