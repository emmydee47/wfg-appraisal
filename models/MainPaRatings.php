...
/**
 * Table class for main_pa_ratings
 */
class MainPaRatings extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'main_pa_ratings';
        $this->TableName = 'main_pa_ratings';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`main_pa_ratings`";
        $this->Dbid = 'DB';
        ...
        $this->rating_type->InputTextType = "text";
        $this->rating_type->IsForeignKey = true; // Foreign key field
        ...
        $this->rating_type->Lookup = new Lookup('rating_type', 'appraisal_ratings', false, 'id', ["rating","","",""], [], [], [], [], [], [], '', '', "`rating`");
        $this->rating_type->DefaultErrorMessage = $Language->phrase("IncorrectField");
        $this->Fields['rating_type'] = &$this->rating_type;

        // rating_value
        $this->rating_value = new DbField(
            'main_pa_ratings',
            'main_pa_ratings',
            'x_rating_value',
            'rating_value',
        ...
        $this->rating_value->InputTextType = "text";
        $this->rating_value->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Fields['rating_value'] = &$this->rating_value;

        // rating_text
        $this->rating_text = new DbField(
            'main_pa_ratings',
            'main_pa_ratings',
            'x_rating_text',
            'rating_text',
        ...
        $this->rating_text->InputTextType = "text";
        $this->Fields['rating_text'] = &$this->rating_text;

        // rating_description
        $this->rating_description = new DbField(
            'main_pa_ratings',
            'main_pa_ratings',
            'x_rating_description',
            'rating_description',
            '`rating_description`',
            '`rating_description`',
            201,
        ...
        $this->rating_description->InputTextType = "text";
        $this->Fields['rating_description'] = &$this->rating_description;
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "appraisal_ratings") {
            if ($this->rating_type->getSessionValue() != "") {
                $masterFilter .= "" . GetForeignKeySql("`id`", $this->rating_type->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "appraisal_ratings") {
            if ($this->rating_type->getSessionValue() != "") {
                $detailFilter .= "" . GetForeignKeySql("`rating_type`", $this->rating_type->getSessionValue(), DATATYPE_NUMBER, "DB");
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "appraisal_ratings":
                $key = $keys["rating_type"] ?? "";
                if (EmptyValue($key)) {
                ...
                if ($validKeys) {
                    return "`id`=" . QuotedValue($keys["rating_type"], $masterTable->id->DataType, $masterTable->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "appraisal_ratings":
                return "`rating_type`=" . QuotedValue($masterTable->id->DbValue, $this->rating_type->DataType, $this->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`main_pa_ratings`";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->rating_text->DbValue = $row['rating_text'];
        $this->rating_description->DbValue = $row['rating_description'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("mainparatingslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "mainparatingsedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "mainparatingsadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "MainPaRatingsView";
            case Config("API_ADD_ACTION"):
                return "MainPaRatingsAdd";
            case Config("API_EDIT_ACTION"):
                return "MainPaRatingsEdit";
            case Config("API_DELETE_ACTION"):
                return "MainPaRatingsDelete";
            case Config("API_LIST_ACTION"):
                return "MainPaRatingsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("mainparatingsview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("mainparatingsview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "mainparatingsadd?" . $this->getUrlParm($parm);
        } else {
            $url = "mainparatingsadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("mainparatingsedit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("mainparatingsadd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("mainparatingsdelete", $this->getUrlParm());
    }

    // Add master url
    public function addMasterUrl($url)
    {
        if ($this->getCurrentMasterTable() == "appraisal_ratings" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id", $this->rating_type->CurrentValue);
        }
        return $url;
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->rating_text->EditValue = $this->rating_text->CurrentValue;
        ...
        $this->rating_description->EditValue = $this->rating_description->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->rating_type);
                    $doc->exportCaption($this->rating_value);
                    $doc->exportCaption($this->rating_text);
                    $doc->exportCaption($this->rating_description);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->rating_type);
                        $doc->exportField($this->rating_value);
                        $doc->exportField($this->rating_text);
                        $doc->exportField($this->rating_description);
