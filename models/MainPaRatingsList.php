 ...
 * Page class
 */
class MainPaRatingsList extends MainPaRatings
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MainPaRatingsList";
    ...
    public $FormName = "fmain_pa_ratingslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "mainparatingsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mainparatingsdelete";
        $this->MultiUpdateUrl = "mainparatingsupdate";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "appraisal_ratings") {
            $masterTbl = Container("appraisal_ratings");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("appraisalratingslist"); // Return to master page
                return;
            } else {
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_rating_type") && $CurrentForm->hasValue("o_rating_type") && $this->rating_type->CurrentValue != $this->rating_type->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_rating_value") && $CurrentForm->hasValue("o_rating_value") && $this->rating_value->CurrentValue != $this->rating_value->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_rating_text") && $CurrentForm->hasValue("o_rating_text") && $this->rating_text->CurrentValue != $this->rating_text->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_rating_description") && $CurrentForm->hasValue("o_rating_description") && $this->rating_description->CurrentValue != $this->rating_description->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->rating_type->clearErrorMessage();
        $this->rating_value->clearErrorMessage();
        $this->rating_text->clearErrorMessage();
        $this->rating_description->clearErrorMessage();
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->rating_type->AdvancedSearch->toJson(), ","); // Field rating_type
        $filterList = Concat($filterList, $this->rating_value->AdvancedSearch->toJson(), ","); // Field rating_value
        $filterList = Concat($filterList, $this->rating_text->AdvancedSearch->toJson(), ","); // Field rating_text
        $filterList = Concat($filterList, $this->rating_description->AdvancedSearch->toJson(), ","); // Field rating_description
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmain_pa_ratingssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field rating_text
        $this->rating_text->AdvancedSearch->SearchValue = @$filter["x_rating_text"];
        $this->rating_text->AdvancedSearch->SearchOperator = @$filter["z_rating_text"];
        $this->rating_text->AdvancedSearch->SearchCondition = @$filter["v_rating_text"];
        $this->rating_text->AdvancedSearch->SearchValue2 = @$filter["y_rating_text"];
        $this->rating_text->AdvancedSearch->SearchOperator2 = @$filter["w_rating_text"];
        $this->rating_text->AdvancedSearch->save();

        // Field rating_description
        $this->rating_description->AdvancedSearch->SearchValue = @$filter["x_rating_description"];
        $this->rating_description->AdvancedSearch->SearchOperator = @$filter["z_rating_description"];
        $this->rating_description->AdvancedSearch->SearchCondition = @$filter["v_rating_description"];
        $this->rating_description->AdvancedSearch->SearchValue2 = @$filter["y_rating_description"];
        $this->rating_description->AdvancedSearch->SearchOperator2 = @$filter["w_rating_description"];
        $this->rating_description->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->rating_text;
        $searchFlds[] = &$this->rating_description;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->rating_text); // rating_text
            $this->updateSort($this->rating_description); // rating_description
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->rating_text->setSort("");
                $this->rating_description->setSort("");
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canEdit()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"main_pa_ratings\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
            ...
            if ($Security->canAdd()) {
                if (IsMobile()) {
                    ...
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"main_pa_ratings\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                }
            } else {
                $opt->Body = "";
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if (IsMobile()) {
            ...
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"main_pa_ratings\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("rating_type", $this->createColumnOption("rating_type"));
            $option->add("rating_value", $this->createColumnOption("rating_value"));
            $option->add("rating_text", $this->createColumnOption("rating_text"));
            $option->add("rating_description", $this->createColumnOption("rating_description"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_pa_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_pa_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_pa_ratingslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fmain_pa_ratingslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                $option->UseDropDownButton = false;
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fmain_pa_ratingslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                    $item = &$option->add("gridcancel");
                    $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_rating_type")) {
            $this->rating_type->setOldValue($CurrentForm->getValue("o_rating_type"));
        }
        ...
        if ($CurrentForm->hasValue("o_rating_value")) {
            $this->rating_value->setOldValue($CurrentForm->getValue("o_rating_value"));
        }
        ...
        if ($CurrentForm->hasValue("o_rating_text")) {
            $this->rating_text->setOldValue($CurrentForm->getValue("o_rating_text"));
        }
        ...
        if ($CurrentForm->hasValue("o_rating_description")) {
            $this->rating_description->setOldValue($CurrentForm->getValue("o_rating_description"));
        }
        ...
        if ($CurrentForm->hasValue("o_modifieddate")) {
            $this->modifieddate->setOldValue($CurrentForm->getValue("o_modifieddate"));
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if (strval($this->rating_value->EditValue) != "" && is_numeric($this->rating_value->EditValue)) {
                ...
                $this->rating_value->OldValue = $this->rating_value->EditValue;
            }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // rating_type
            if ($this->rating_type->getSessionValue() != "") {
                $this->rating_type->ReadOnly = true;
            }
            ...
            // rating_text
            $this->rating_text->setDbValueDef($rsnew, $this->rating_text->CurrentValue, null, $this->rating_text->ReadOnly);

            // rating_description
            $this->rating_description->setDbValueDef($rsnew, $this->rating_description->CurrentValue, null, $this->rating_description->ReadOnly);
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['rating_type']); // rating_type
        $hash .= GetFieldHash($row['rating_value']); // rating_value
        $hash .= GetFieldHash($row['rating_text']); // rating_text
        $hash .= GetFieldHash($row['rating_description']); // rating_description
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmain_pa_ratingslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fmain_pa_ratingssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "appraisal_ratings") {
            $appraisal_ratings = Container("appraisal_ratings");
            $rsmaster = $appraisal_ratings->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $appraisal_ratings;
                    $appraisal_ratings->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
