 ...
 * Page class
 */
class MainPaEmployeeRatingsList extends MainPaEmployeeRatings
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MainPaEmployeeRatingsList";
    ...
    public $FormName = "fmain_pa_employee_ratingslist";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "mainpaemployeeratingsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mainpaemployeeratingsdelete";
        $this->MultiUpdateUrl = "mainpaemployeeratingsupdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->appraisal_id->AdvancedSearch->toJson(), ","); // Field appraisal_id
        $filterList = Concat($filterList, $this->employee_id->AdvancedSearch->toJson(), ","); // Field employee_id
        $filterList = Concat($filterList, $this->consolidated_rating->AdvancedSearch->toJson(), ","); // Field consolidated_rating
        $filterList = Concat($filterList, $this->appraisal_status->AdvancedSearch->toJson(), ","); // Field appraisal_status
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmain_pa_employee_ratingssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field appraisal_id
        $this->appraisal_id->AdvancedSearch->SearchValue = @$filter["x_appraisal_id"];
        $this->appraisal_id->AdvancedSearch->SearchOperator = @$filter["z_appraisal_id"];
        $this->appraisal_id->AdvancedSearch->SearchCondition = @$filter["v_appraisal_id"];
        $this->appraisal_id->AdvancedSearch->SearchValue2 = @$filter["y_appraisal_id"];
        $this->appraisal_id->AdvancedSearch->SearchOperator2 = @$filter["w_appraisal_id"];
        $this->appraisal_id->AdvancedSearch->save();
        ...
        $this->consolidated_rating->AdvancedSearch->SearchValue = @$filter["x_consolidated_rating"];
        $this->consolidated_rating->AdvancedSearch->SearchOperator = @$filter["z_consolidated_rating"];
        $this->consolidated_rating->AdvancedSearch->SearchCondition = @$filter["v_consolidated_rating"];
        $this->consolidated_rating->AdvancedSearch->SearchValue2 = @$filter["y_consolidated_rating"];
        $this->consolidated_rating->AdvancedSearch->SearchOperator2 = @$filter["w_consolidated_rating"];
        $this->consolidated_rating->AdvancedSearch->save();

        // Field appraisal_status
        $this->appraisal_status->AdvancedSearch->SearchValue = @$filter["x_appraisal_status"];
        $this->appraisal_status->AdvancedSearch->SearchOperator = @$filter["z_appraisal_status"];
        $this->appraisal_status->AdvancedSearch->SearchCondition = @$filter["v_appraisal_status"];
        $this->appraisal_status->AdvancedSearch->SearchValue2 = @$filter["y_appraisal_status"];
        $this->appraisal_status->AdvancedSearch->SearchOperator2 = @$filter["w_appraisal_status"];
        $this->appraisal_status->AdvancedSearch->save();
    ...
    protected function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->appraisal_id, $default, false); // appraisal_id
        $this->buildSearchSql($where, $this->employee_id, $default, false); // employee_id
        $this->buildSearchSql($where, $this->consolidated_rating, $default, false); // consolidated_rating
        $this->buildSearchSql($where, $this->appraisal_status, $default, false); // appraisal_status
        ...
        if (!$default && $this->Command == "search") {
            $this->appraisal_id->AdvancedSearch->save(); // appraisal_id
            $this->employee_id->AdvancedSearch->save(); // employee_id
            $this->consolidated_rating->AdvancedSearch->save(); // consolidated_rating
            $this->appraisal_status->AdvancedSearch->save(); // appraisal_status
    ...
    {
        if ($this->appraisal_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->employee_id->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->consolidated_rating->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->appraisal_status->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    {
        $this->appraisal_id->AdvancedSearch->unsetSession();
        $this->employee_id->AdvancedSearch->unsetSession();
        $this->consolidated_rating->AdvancedSearch->unsetSession();
        $this->appraisal_status->AdvancedSearch->unsetSession();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->consolidated_rating); // consolidated_rating
            $this->updateSort($this->appraisal_status); // appraisal_status
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->consolidated_rating->setSort("");
                $this->appraisal_status->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_employee_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_employee_ratingslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("consolidated_rating", $this->createColumnOption("consolidated_rating"));
            $option->add("appraisal_status", $this->createColumnOption("appraisal_status"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_pa_employee_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_pa_employee_ratingssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_pa_employee_ratingslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->appraisal_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->appraisal_id->AdvancedSearch->SearchValue != "" || $this->appraisal_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->consolidated_rating->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->consolidated_rating->AdvancedSearch->SearchValue != "" || $this->consolidated_rating->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->appraisal_status->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->appraisal_status->AdvancedSearch->SearchValue != "" || $this->appraisal_status->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmain_pa_employee_ratingslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        if (IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" href=\"mainpaemployeeratingssearch\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch") . "\" data-table=\"main_pa_employee_ratings\" data-caption=\"" . $Language->phrase("AdvancedSearch") . "\" data-ew-action=\"modal\" data-url=\"mainpaemployeeratingssearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearchBtn") . "</a>";
        }
        $item->Visible = true;
    ...
    */
    ...
    {
        $item = &$this->ListOptions->add("resend_notification");
         $item->Header = ""; // Set the column header (for List page)
         $item->moveTo(0); // Move the column to the specified index
         $item = &$this->ListOptions->add("view_appraisal");
         $item->Header = ""; // Set the column header (for List page)
    ...
    {
        // Example:
        $appraisal_id = $this->appraisal_id->CurrentValue;
        $user_id = $this->employee_id->CurrentValue;
        $this->ListOptions["resend_notification"]->Body = "<button type='button' data-appraisal='".$appraisal_id."' data-employee='".$user_id."'  class='btn btn-xs btn-primary btn-notify-l1'>Notify L1</button>";
        $group_id = $this->group_id->CurrentValue;
        $this->ListOptions["view_appraisal"]->Body = '<span id="el5_main_pa_employee_ratings_view" class="main_pa_employee_ratings_view"><a class="ew-row-link ew-view" title="" data-caption="View" href="appraisalreport/?appraisal_id='.$appraisal_id.'&user_id='.$user_id.'&group_id='.$group_id.'" data-original-title="View"><i data-phrase="ViewLink" class="icon-view ew-icon" data-caption="View"></i></a></span>';
    }
