...
class MainUsersEdit extends MainUsers
{
    ...
    protected function validateForm()
    {
        ...
        if ($this->contactnumber->Required) {
            if (!$this->contactnumber->IsDetailKey && EmptyValue($this->contactnumber->FormValue)) {
                $this->contactnumber->addErrorMessage(str_replace("%s", $this->contactnumber->caption(), $this->contactnumber->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // emprole
            if ($Security->canAdmin()) { // System admin
                $this->emprole->setDbValueDef($rsnew, $this->emprole->CurrentValue, null, $this->emprole->ReadOnly);
            }
            ...
            // contactnumber
            $this->contactnumber->setDbValueDef($rsnew, $this->contactnumber->CurrentValue, null, $this->contactnumber->ReadOnly);

            // backgroundchk_status
            $this->backgroundchk_status->setDbValueDef($rsnew, $this->backgroundchk_status->CurrentValue, null, $this->backgroundchk_status->ReadOnly);

            // emppassword
            $this->emppassword->setDbValueDef($rsnew, $this->emppassword->CurrentValue, null, $this->emppassword->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['emppassword'] == $this->emppassword->CurrentValue);

            // profileimg
            if ($this->profileimg->Visible && !$this->profileimg->ReadOnly && !$this->profileimg->Upload->KeepFile) {
                $this->profileimg->Upload->DbValue = $rsold['profileimg']; // Get original value
            ...
            if ($this->profileimg->Visible && !$this->profileimg->Upload->KeepFile) {
                ...
                if (!EmptyValue($this->profileimg->Upload->FileName)) {
                    ...
                    $this->profileimg->setDbValueDef($rsnew, $this->profileimg->Upload->FileName, null, $this->profileimg->ReadOnly);
                }
            }
