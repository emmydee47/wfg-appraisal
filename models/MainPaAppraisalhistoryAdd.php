 ...
 * Page class
 */
class MainPaAppraisalhistoryAdd extends MainPaAppraisalhistory
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'main_pa_appraisalhistory';

    // Page object name
    public $PageObjName = "MainPaAppraisalhistoryAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (main_pa_appraisalhistory)
        if (!isset($GLOBALS["main_pa_appraisalhistory"]) || get_class($GLOBALS["main_pa_appraisalhistory"]) == PROJECT_NAMESPACE . "main_pa_appraisalhistory") {
            $GLOBALS["main_pa_appraisalhistory"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'main_pa_appraisalhistory');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("main_pa_appraisalhistory");
                $doc = new $class($tbl);
                $doc->Text = @$content;
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "mainpaappraisalhistoryview") {
                        $row["view"] = "1";
                    }
    ...
    public function run()
    {
        ...
        $this->pa_initialization_id->setVisibility();
        ...
        $this->setupLookupOptions($this->pa_initialization_id);
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("mainpaappraisalhistorylist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "mainpaappraisalhistorylist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "mainpaappraisalhistoryview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->pa_initialization_id->CurrentValue = null;
        $this->pa_initialization_id->OldValue = $this->pa_initialization_id->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'pa_initialization_id' first before field var 'x_pa_initialization_id'
        $val = $CurrentForm->hasValue("pa_initialization_id") ? $CurrentForm->getValue("pa_initialization_id") : $CurrentForm->getValue("x_pa_initialization_id");
        if (!$this->pa_initialization_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->pa_initialization_id->Visible = false; // Disable update for API request
            } else {
                $this->pa_initialization_id->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->pa_initialization_id->CurrentValue = $this->pa_initialization_id->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['pa_initialization_id'] = $this->pa_initialization_id->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        $this->pa_initialization_id->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // pa_initialization_id
            $curVal = strval($this->pa_initialization_id->CurrentValue);
            if ($curVal != "") {
                $this->pa_initialization_id->ViewValue = $this->pa_initialization_id->lookupCacheOption($curVal);
                if ($this->pa_initialization_id->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->pa_initialization_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->pa_initialization_id->Lookup->renderViewRow($rswrk[0]);
                        $this->pa_initialization_id->ViewValue = $this->pa_initialization_id->displayValue($arwrk);
                    } else {
                        $this->pa_initialization_id->ViewValue = $this->pa_initialization_id->CurrentValue;
                    }
                }
            } else {
                $this->pa_initialization_id->ViewValue = null;
            }
            ...
            $curVal = trim(strval($this->pa_initialization_id->CurrentValue));
            if ($curVal != "") {
                $this->pa_initialization_id->ViewValue = $this->pa_initialization_id->lookupCacheOption($curVal);
            } else {
                $this->pa_initialization_id->ViewValue = $this->pa_initialization_id->Lookup !== null && is_array($this->pa_initialization_id->lookupOptions()) ? $curVal : null;
            }
            if ($this->pa_initialization_id->ViewValue !== null) { // Load from cache
                $this->pa_initialization_id->EditValue = array_values($this->pa_initialization_id->lookupOptions());
                if ($this->pa_initialization_id->ViewValue == "") {
                    $this->pa_initialization_id->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->pa_initialization_id->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->pa_initialization_id->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                ...
                foreach ($arwrk as &$row) {
                    $row = $this->pa_initialization_id->Lookup->renderViewRow($row);
                }
                $this->pa_initialization_id->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->pa_initialization_id->Required) {
            if (!$this->pa_initialization_id->IsDetailKey && EmptyValue($this->pa_initialization_id->FormValue)) {
                $this->pa_initialization_id->addErrorMessage(str_replace("%s", $this->pa_initialization_id->caption(), $this->pa_initialization_id->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // pa_initialization_id
        $this->pa_initialization_id->setDbValueDef($rsnew, $this->pa_initialization_id->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("mainpaappraisalhistorylist"), "", $this->TableVar, true);
