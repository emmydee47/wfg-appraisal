 ...
 * Page class
 */
class MainPaGroupsEmployeesList extends MainPaGroupsEmployees
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'main_pa_groups_employees';

    // Page object name
    public $PageObjName = "MainPaGroupsEmployeesList";
    ...
    public $FormName = "fmain_pa_groups_employeeslist";
    ...
    public function __construct()
    {
        ...
        // Table object (main_pa_groups_employees)
        if (!isset($GLOBALS["main_pa_groups_employees"]) || get_class($GLOBALS["main_pa_groups_employees"]) == PROJECT_NAMESPACE . "main_pa_groups_employees") {
            $GLOBALS["main_pa_groups_employees"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "mainpagroupsemployeesadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mainpagroupsemployeesdelete";
        $this->MultiUpdateUrl = "mainpagroupsemployeesupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'main_pa_groups_employees');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("main_pa_groups_employees");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "main_pa_groups") {
            $masterTbl = Container("main_pa_groups");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("mainpagroupslist"); // Return to master page
                return;
            } else {
    ...
    public function resetFormError()
    {
        ...
        $this->group_id->clearErrorMessage();
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmain_pa_groups_employeessrch", $filters);
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_groups_employeeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_pa_groups_employeeslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_pa_groups_employeessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_pa_groups_employeessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_pa_groups_employeeslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fmain_pa_groups_employeeslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                $option->UseDropDownButton = false;
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridSaveLink")) . "\" form=\"fmain_pa_groups_employeeslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridSaveLink") . "</button>";
                    $item = &$option->add("gridcancel");
                    $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->group_id->getSessionValue() != "") {
                $this->group_id->CurrentValue = GetForeignKeyValue($this->group_id->getSessionValue());
                $this->group_id->OldValue = $this->group_id->CurrentValue;
                $curVal = strval($this->group_id->CurrentValue);
                if ($curVal != "") {
                    $this->group_id->ViewValue = $this->group_id->lookupCacheOption($curVal);
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // Check referential integrity for master table 'main_pa_groups'
            $detailKeys = [];
            $keyValue = $rsnew['group_id'] ?? $rsold['group_id'];
            $detailKeys['group_id'] = $keyValue;
            $masterTable = Container("main_pa_groups");
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "main_pa_groups", $Language->phrase("RelatedRecordRequired"));
    ...
    public function getRowHash(&$rs)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['group_id']); // group_id
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'main_pa_groups_employees'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["group_id"] = $this->group_id->CurrentValue;
        $masterTable = Container("main_pa_groups");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmain_pa_groups_employeeslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fmain_pa_groups_employeessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "main_pa_groups") {
            $main_pa_groups = Container("main_pa_groups");
            $rsmaster = $main_pa_groups->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $main_pa_groups;
                    $main_pa_groups->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "main_pa_groups") {
                $validMaster = true;
                $masterTbl = Container("main_pa_groups");
                if (($parm = Get("fk_id", Get("group_id"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->group_id->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->group_id->setSessionValue($this->group_id->QueryStringValue);
            ...
            if ($masterTblVar == "main_pa_groups") {
                $validMaster = true;
                $masterTbl = Container("main_pa_groups");
                if (($parm = Post("fk_id", Post("group_id"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->group_id->setFormValue($masterTbl->id->FormValue);
                    $this->group_id->setSessionValue($this->group_id->FormValue);
