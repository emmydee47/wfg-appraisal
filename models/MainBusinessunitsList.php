 ...
 * Page class
 */
class MainBusinessunitsList extends MainBusinessunits
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'main_businessunits';

    // Page object name
    public $PageObjName = "MainBusinessunitsList";
    ...
    public $FormName = "fmain_businessunitslist";
    ...
    public function __construct()
    {
        ...
        // Table object (main_businessunits)
        if (!isset($GLOBALS["main_businessunits"]) || get_class($GLOBALS["main_businessunits"]) == PROJECT_NAMESPACE . "main_businessunits") {
            $GLOBALS["main_businessunits"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "mainbusinessunitsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "mainbusinessunitsdelete";
        $this->MultiUpdateUrl = "mainbusinessunitsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'main_businessunits');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("main_businessunits");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->unitcode->setVisibility();
        $this->description->Visible = false;
        $this->unithead->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->unithead);
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->unitcode->AdvancedSearch->toJson(), ","); // Field unitcode
        $filterList = Concat($filterList, $this->description->AdvancedSearch->toJson(), ","); // Field description
        $filterList = Concat($filterList, $this->unithead->AdvancedSearch->toJson(), ","); // Field unithead
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fmain_businessunitssrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field unitcode
        $this->unitcode->AdvancedSearch->SearchValue = @$filter["x_unitcode"];
        $this->unitcode->AdvancedSearch->SearchOperator = @$filter["z_unitcode"];
        $this->unitcode->AdvancedSearch->SearchCondition = @$filter["v_unitcode"];
        $this->unitcode->AdvancedSearch->SearchValue2 = @$filter["y_unitcode"];
        $this->unitcode->AdvancedSearch->SearchOperator2 = @$filter["w_unitcode"];
        $this->unitcode->AdvancedSearch->save();
        ...
        // Field unithead
        $this->unithead->AdvancedSearch->SearchValue = @$filter["x_unithead"];
        $this->unithead->AdvancedSearch->SearchOperator = @$filter["z_unithead"];
        $this->unithead->AdvancedSearch->SearchCondition = @$filter["v_unithead"];
        $this->unithead->AdvancedSearch->SearchValue2 = @$filter["y_unithead"];
        $this->unithead->AdvancedSearch->SearchOperator2 = @$filter["w_unithead"];
        $this->unithead->AdvancedSearch->save();
    ...
    protected function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->unitname;
        $searchFlds[] = &$this->unitcode;
        $searchFlds[] = &$this->description;
        $searchFlds[] = &$this->unithead;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->unitname); // unitname
            $this->updateSort($this->unitcode); // unitcode
            $this->updateSort($this->unithead); // unithead
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->unitcode->setSort("");
                $this->description->setSort("");
                $this->unithead->setSort("");
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_businessunitslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_businessunitslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("unitname", $this->createColumnOption("unitname"));
            $option->add("unitcode", $this->createColumnOption("unitcode"));
            $option->add("unithead", $this->createColumnOption("unithead"));
            $option->add("createdby", $this->createColumnOption("createdby"));
            $option->add("modifiedby", $this->createColumnOption("modifiedby"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_businessunitssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_businessunitssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_businessunitslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->unitcode->setDbValue($row['unitcode']);
        $this->description->setDbValue($row['description']);
        $this->unithead->setDbValue($row['unithead']);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // unitcode
            $this->unitcode->ViewValue = $this->unitcode->CurrentValue;
            $this->unitcode->ViewCustomAttributes = "";

            // unithead
            $curVal = strval($this->unithead->CurrentValue);
            if ($curVal != "") {
                $this->unithead->ViewValue = $this->unithead->lookupCacheOption($curVal);
                if ($this->unithead->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $sqlWrk = $this->unithead->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->unithead->Lookup->renderViewRow($rswrk[0]);
                        $this->unithead->ViewValue = $this->unithead->displayValue($arwrk);
                    } else {
                        $this->unithead->ViewValue = $this->unithead->CurrentValue;
                    }
                }
            } else {
                $this->unithead->ViewValue = null;
            }
            $this->unithead->ViewCustomAttributes = "";
            ...
            // unitcode
            $this->unitcode->LinkCustomAttributes = "";
            $this->unitcode->HrefValue = "";
            $this->unitcode->TooltipValue = "";

            // unithead
            $this->unithead->LinkCustomAttributes = "";
            $this->unithead->HrefValue = "";
            $this->unithead->TooltipValue = "";
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" form="fmain_businessunitslist" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmailText") . '" data-sel="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-bs-toggle=\"button\" data-form=\"fmain_businessunitssrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
