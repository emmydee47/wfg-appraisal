 ...
 * Page class
 */
class MessageTemplatesAdd extends MessageTemplates
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "MessageTemplatesAdd";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page
                    ...
                    if ($pageName == "messagetemplatesview") {
                        $row["view"] = "1";
                    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("messagetemplateslist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "messagetemplateslist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "messagetemplatesview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->recepient_group->CurrentValue = null;
        $this->recepient_group->OldValue = $this->recepient_group->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'recepient_group' first before field var 'x_recepient_group'
        $val = $CurrentForm->hasValue("recepient_group") ? $CurrentForm->getValue("recepient_group") : $CurrentForm->getValue("x_recepient_group");
        if (!$this->recepient_group->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->recepient_group->Visible = false; // Disable update for API request
            } else {
                $this->recepient_group->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->recepient_group->CurrentValue = $this->recepient_group->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['recepient_group'] = $this->recepient_group->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        // recepient_group
        $this->recepient_group->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // recepient_group
            $this->recepient_group->setupEditAttributes();
            $this->recepient_group->EditCustomAttributes = "";
            $this->recepient_group->EditValue = $this->recepient_group->options(true);
            $this->recepient_group->PlaceHolder = RemoveHtml($this->recepient_group->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->recepient_group->Required) {
            if (!$this->recepient_group->IsDetailKey && EmptyValue($this->recepient_group->FormValue)) {
                $this->recepient_group->addErrorMessage(str_replace("%s", $this->recepient_group->caption(), $this->recepient_group->RequiredErrorMessage));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // recepient_group
        $this->recepient_group->setDbValueDef($rsnew, $this->recepient_group->CurrentValue, null, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("messagetemplateslist"), "", $this->TableVar, true);
