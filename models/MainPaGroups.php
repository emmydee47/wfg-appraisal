...
/**
 * Table class for main_pa_groups
 */
class MainPaGroups extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'main_pa_groups';
        $this->TableName = 'main_pa_groups';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`main_pa_groups`";
        $this->Dbid = 'DB';
        ...
        $this->business_unit->Lookup = new Lookup('business_unit', 'main_businessunits', false, 'id', ["unitname","unitcode","",""], [], [], [], [], [], [], '', '', "CONCAT(COALESCE(`unitname`, ''),'" . ValueSeparator(1, $this->business_unit) . "',COALESCE(`unitcode`,''))");
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "main_pa_groups_employees") {
            $detailUrl = Container("main_pa_groups_employees")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "mainpagroupslist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`main_pa_groups`";
    }
    ...
    {
        // Cascade Update detail table 'main_pa_groups_employees'
        $cascadeUpdate = false;
        $rscascade = [];
        if ($rsold && (isset($rs['id']) && $rsold['id'] != $rs['id'])) { // Update detail field 'group_id'
            $cascadeUpdate = true;
            $rscascade['group_id'] = $rs['id'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("main_pa_groups_employees")->loadRs("`group_id` = " . QuotedValue($rsold['id'], DATATYPE_NUMBER, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $rsdtlold) {
                ...
                $success = Container("main_pa_groups_employees")->rowUpdating($rsdtlold, $rsdtlnew);
                if ($success) {
                    $success = Container("main_pa_groups_employees")->update($rscascade, $rskey, $rsdtlold);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("main_pa_groups_employees")->rowUpdated($rsdtlold, $rsdtlnew);
            }
        }
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        // Cascade delete detail table 'main_pa_groups_employees'
        $dtlrows = Container("main_pa_groups_employees")->loadRs("`group_id` = " . QuotedValue($rs['id'], DATATYPE_NUMBER, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($dtlrows as $dtlrow) {
            $success = Container("main_pa_groups_employees")->rowDeleting($dtlrow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                $success = Container("main_pa_groups_employees")->delete($dtlrow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($dtlrows as $dtlrow) {
                Container("main_pa_groups_employees")->rowDeleted($dtlrow);
            }
        }
        if ($success) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("mainpagroupslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "mainpagroupsedit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "mainpagroupsadd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "MainPaGroupsView";
            case Config("API_ADD_ACTION"):
                return "MainPaGroupsAdd";
            case Config("API_EDIT_ACTION"):
                return "MainPaGroupsEdit";
            case Config("API_DELETE_ACTION"):
                return "MainPaGroupsDelete";
            case Config("API_LIST_ACTION"):
                return "MainPaGroupsList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("mainpagroupsview", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("mainpagroupsview", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "mainpagroupsadd?" . $this->getUrlParm($parm);
        } else {
            $url = "mainpagroupsadd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("mainpagroupsedit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("mainpagroupsedit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("mainpagroupsadd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("mainpagroupsadd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("mainpagroupsdelete", $this->getUrlParm());
    }
