 ...
 * Page class
 */
class MainGroupPaQuestionsList extends MainGroupPaQuestions
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'main_group_pa_questions';

    // Page object name
    public $PageObjName = "MainGroupPaQuestionsList";
    ...
    public $FormName = "fmain_group_pa_questionslist";
    ...
    public function __construct()
    {
        ...
        // Table object (main_group_pa_questions)
        if (!isset($GLOBALS["main_group_pa_questions"]) || get_class($GLOBALS["main_group_pa_questions"]) == PROJECT_NAMESPACE . "main_group_pa_questions") {
            $GLOBALS["main_group_pa_questions"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "maingrouppaquestionsadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "maingrouppaquestionsdelete";
        $this->MultiUpdateUrl = "maingrouppaquestionsupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'main_group_pa_questions');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $tbl = Container("main_group_pa_questions");
                $doc = new $class($tbl);
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->setupLookupOptions($this->question);
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "main_pa_initialization") {
            $masterTbl = Container("main_pa_initialization");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("mainpainitializationlist"); // Return to master page
                return;
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "main_pa_questions") {
            $masterTbl = Container("main_pa_questions");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("mainpaquestionslist"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_appraisal_id") && $CurrentForm->hasValue("o_appraisal_id") && $this->appraisal_id->CurrentValue != $this->appraisal_id->OldValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_question") && $CurrentForm->hasValue("o_question") && $this->question->CurrentValue != $this->question->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->appraisal_id->clearErrorMessage();
        $this->business_unit->clearErrorMessage();
        $this->group->clearErrorMessage();
        $this->question->clearErrorMessage();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->appraisal_id); // appraisal_id
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->appraisal_id->setSessionValue("");
                        $this->question->setSessionValue("");
            }
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_group_pa_questionslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fmain_group_pa_questionslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("appraisal_id", $this->createColumnOption("appraisal_id"));
            $option->add("business_unit", $this->createColumnOption("business_unit"));
            $option->add("group", $this->createColumnOption("group"));
            $option->add("question", $this->createColumnOption("question"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmain_group_pa_questionssrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmain_group_pa_questionssrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmain_group_pa_questionslist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
            ...
            if ($this->isGridAdd()) {
                ...
                $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($Language->phrase("GridInsertLink")) . "\" form=\"fmain_group_pa_questionslist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $Language->phrase("GridInsertLink") . "</button>";
                // Add grid cancel
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->appraisal_id->CurrentValue = isset($_GET['appraisal_id'])?$_GET['appraisal_id']:'';
        $this->appraisal_id->OldValue = $this->appraisal_id->CurrentValue;
        $this->business_unit->CurrentValue = isset($_GET['business_unit'])?$_GET['business_unit']:'';
        $this->business_unit->OldValue = $this->business_unit->CurrentValue;
        $this->group->CurrentValue = isset($_GET['group_id'])?$_GET['group_id']:'';
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_appraisal_id")) {
            $this->appraisal_id->setOldValue($CurrentForm->getValue("o_appraisal_id"));
        }
        ...
        if ($CurrentForm->hasValue("o_business_unit")) {
            $this->business_unit->setOldValue($CurrentForm->getValue("o_business_unit"));
        }
        ...
        if ($CurrentForm->hasValue("o_question")) {
            $this->question->setOldValue($CurrentForm->getValue("o_question"));
        }
    }
    ...
    protected function newRow()
    {
        ...
        $row['appraisal_id'] = $this->appraisal_id->CurrentValue;
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                if ($this->business_unit->ViewValue === null) { // Lookup from database
                    $filterWrk = "`business_unit`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
            ...
            // question
            $curVal = strval($this->question->CurrentValue);
            if ($curVal != "") {
                $this->question->ViewValue = $this->question->lookupCacheOption($curVal);
                if ($this->question->ViewValue === null) { // Lookup from database
                    $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                    $lookupFilter = function() {
                        return "`group`='".$_GET['group_id']."'";
                    };
                    $lookupFilter = $lookupFilter->bindTo($this);
                    $sqlWrk = $this->question->Lookup->getSql(false, $filterWrk, $lookupFilter, $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->question->Lookup->renderViewRow($rswrk[0]);
                        $this->question->ViewValue = $this->question->displayValue($arwrk);
                    } else {
                        $this->question->ViewValue = $this->question->CurrentValue;
                    }
                }
            } else {
                $this->question->ViewValue = null;
            }
            ...
            if ($this->appraisal_id->getSessionValue() != "") {
                $this->appraisal_id->CurrentValue = GetForeignKeyValue($this->appraisal_id->getSessionValue());
                $this->appraisal_id->OldValue = $this->appraisal_id->CurrentValue;
                $this->appraisal_id->ViewValue = $this->appraisal_id->CurrentValue;
                $curVal = strval($this->appraisal_id->CurrentValue);
                if ($curVal != "") {
                ...
                $this->appraisal_id->EditValue = HtmlEncode($this->appraisal_id->CurrentValue);
                $curVal = strval($this->appraisal_id->CurrentValue);
                if ($curVal != "") {
                    $this->appraisal_id->EditValue = $this->appraisal_id->lookupCacheOption($curVal);
                    if ($this->appraisal_id->EditValue === null) { // Lookup from database
                        $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                        $sqlWrk = $this->appraisal_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                        $conn = Conn();
                        $config = $conn->getConfiguration();
                        ...
                        if ($ari > 0) { // Lookup values found
                            $arwrk = $this->appraisal_id->Lookup->renderViewRow($rswrk[0]);
                            $this->appraisal_id->EditValue = $this->appraisal_id->displayValue($arwrk);
                        } else {
                            $this->appraisal_id->EditValue = HtmlEncode($this->appraisal_id->CurrentValue);
                        }
                    }
                } else {
                    $this->appraisal_id->EditValue = null;
                }
            ...
            if ($this->business_unit->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`business_unit`" . SearchString("=", $this->business_unit->CurrentValue, DATATYPE_NUMBER, "");
                }
            ...
            if ($this->question->getSessionValue() != "") {
                $this->question->CurrentValue = GetForeignKeyValue($this->question->getSessionValue());
                $this->question->OldValue = $this->question->CurrentValue;
                $curVal = strval($this->question->CurrentValue);
                if ($curVal != "") {
                    $this->question->ViewValue = $this->question->lookupCacheOption($curVal);
                ...
                $curVal = trim(strval($this->question->CurrentValue));
                if ($curVal != "") {
                    $this->question->ViewValue = $this->question->lookupCacheOption($curVal);
                } else {
                    $this->question->ViewValue = $this->question->Lookup !== null && is_array($this->question->lookupOptions()) ? $curVal : null;
                }
                if ($this->question->ViewValue !== null) { // Load from cache
                    $this->question->EditValue = array_values($this->question->lookupOptions());
                    if ($this->question->ViewValue == "") {
                        $this->question->ViewValue = $Language->phrase("PleaseSelect");
                    }
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`id`" . SearchString("=", $this->question->CurrentValue, DATATYPE_NUMBER, "");
                    }
                    $lookupFilter = function() {
                        return "`group`='".$_GET['group_id']."'";
                    };
                    $lookupFilter = $lookupFilter->bindTo($this);
                    $sqlWrk = $this->question->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    ...
                    foreach ($arwrk as &$row) {
                        $row = $this->question->Lookup->renderViewRow($row);
                    }
                    $this->question->EditValue = $arwrk;
                }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'main_group_pa_questions'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["appraisal_id"] = $this->appraisal_id->CurrentValue;
        $masterTable = Container("main_pa_initialization");
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = str_replace("%t", "main_pa_initialization", $Language->phrase("RelatedRecordRequired"));
        ...
        // appraisal_id
        $this->appraisal_id->setDbValueDef($rsnew, $this->appraisal_id->CurrentValue, 0, strval($this->appraisal_id->CurrentValue) == "");

        // business_unit
        $this->business_unit->setDbValueDef($rsnew, $this->business_unit->CurrentValue, 0, false);

        // group
        $this->group->setDbValueDef($rsnew, $this->group->CurrentValue, 0, false);

        // question
        $this->question->setDbValueDef($rsnew, $this->question->CurrentValue, 0, false);
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "main_pa_initialization") {
                $validMaster = true;
                $masterTbl = Container("main_pa_initialization");
                if (($parm = Get("fk_id", Get("appraisal_id"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->appraisal_id->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->appraisal_id->setSessionValue($this->appraisal_id->QueryStringValue);
            ...
            if ($masterTblVar == "main_pa_questions") {
                $validMaster = true;
                $masterTbl = Container("main_pa_questions");
                if (($parm = Get("fk_id", Get("question"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->question->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->question->setSessionValue($this->question->QueryStringValue);
                    if (!is_numeric($masterTbl->id->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
            ...
            if ($masterTblVar == "main_pa_initialization") {
                $validMaster = true;
                $masterTbl = Container("main_pa_initialization");
                if (($parm = Post("fk_id", Post("appraisal_id"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->appraisal_id->setFormValue($masterTbl->id->FormValue);
                    $this->appraisal_id->setSessionValue($this->appraisal_id->FormValue);
            ...
            if ($masterTblVar == "main_pa_questions") {
                $validMaster = true;
                $masterTbl = Container("main_pa_questions");
                if (($parm = Post("fk_id", Post("question"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->question->setFormValue($masterTbl->id->FormValue);
                    $this->question->setSessionValue($this->question->FormValue);
                    if (!is_numeric($masterTbl->id->FormValue)) {
                        $validMaster = false;
                    }
                } else {
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "main_pa_initialization") {
                if ($this->appraisal_id->CurrentValue == "") {
                    $this->appraisal_id->setSessionValue("");
                }
            }
            if ($masterTblVar != "main_pa_questions") {
                if ($this->question->CurrentValue == "") {
                    $this->question->setSessionValue("");
                }
            }
        }
