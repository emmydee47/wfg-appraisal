...
// Page object
$MainPaGroupsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { main_pa_groups: currentTable } });
var currentForm, currentPageID;
var fmain_pa_groupsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmain_pa_groupsedit = new ew.Form("fmain_pa_groupsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmain_pa_groupsedit;

    // Add fields
    var fields = currentTable.fields;
    fmain_pa_groupsedit.addFields([
    ...
    // Form_CustomValidate
    fmain_pa_groupsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmain_pa_groupsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmain_pa_groupsedit.lists.business_unit = <?= $Page->business_unit->toClientList($Page) ?>;
    fmain_pa_groupsedit.lists.createdby = <?= $Page->createdby->toClientList($Page) ?>;
    fmain_pa_groupsedit.lists.modifiedby = <?= $Page->modifiedby->toClientList($Page) ?>;
    loadjs.done("fmain_pa_groupsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmain_pa_groupsedit" id="fmain_pa_groupsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_main_pa_groups_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_main_pa_groups_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="main_pa_groups" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_main_pa_groups_business_unit">
    <select
        ...
        data-select2-id="fmain_pa_groupsedit_x_business_unit"
...
<script>
loadjs.ready("fmain_pa_groupsedit", function() {
    var options = { name: "x_business_unit", selectId: "fmain_pa_groupsedit_x_business_unit" };
    if (fmain_pa_groupsedit.lists.business_unit.lookupOptions.length) {
        options.data = { id: "x_business_unit", form: "fmain_pa_groupsedit" };
    } else {
        options.ajax = { id: "x_business_unit", form: "fmain_pa_groupsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php
    if (in_array("main_pa_groups_employees", explode(",", $Page->getCurrentDetailTable())) && $main_pa_groups_employees->DetailEdit) {
?>
