...
// Page object
$MainPaRatingsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { main_pa_ratings: currentTable } });
var currentForm, currentPageID;
var fmain_pa_ratingsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmain_pa_ratingsedit = new ew.Form("fmain_pa_ratingsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmain_pa_ratingsedit;

    // Add fields
    var fields = currentTable.fields;
    fmain_pa_ratingsedit.addFields([
    ...
    // Form_CustomValidate
    fmain_pa_ratingsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmain_pa_ratingsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmain_pa_ratingsedit.lists.rating_type = <?= $Page->rating_type->toClientList($Page) ?>;
    loadjs.done("fmain_pa_ratingsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmain_pa_ratingsedit" id="fmain_pa_ratingsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_main_pa_ratings_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_main_pa_ratings_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="main_pa_ratings" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_main_pa_ratings_rating_type">
    <select
        ...
        data-select2-id="fmain_pa_ratingsedit_x_rating_type"
...
<script>
loadjs.ready("fmain_pa_ratingsedit", function() {
    var options = { name: "x_rating_type", selectId: "fmain_pa_ratingsedit_x_rating_type" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmain_pa_ratingsedit.lists.rating_type.lookupOptions.length) {
        options.data = { id: "x_rating_type", form: "fmain_pa_ratingsedit" };
    } else {
        options.ajax = { id: "x_rating_type", form: "fmain_pa_ratingsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fmain_pa_ratingsedit", "editor"], function() {
	ew.createEditor("fmain_pa_ratingsedit", "x_rating_description", 35, 4, <?= $Page->rating_description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
