...
// Page object
$MainPaEmployeeRatingsEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { main_pa_employee_ratings: currentTable } });
var currentForm, currentPageID;
var fmain_pa_employee_ratingsedit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fmain_pa_employee_ratingsedit = new ew.Form("fmain_pa_employee_ratingsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fmain_pa_employee_ratingsedit;

    // Add fields
    var fields = currentTable.fields;
    fmain_pa_employee_ratingsedit.addFields([
    ...
    // Form_CustomValidate
    fmain_pa_employee_ratingsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmain_pa_employee_ratingsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fmain_pa_employee_ratingsedit.lists.appraisal_id = <?= $Page->appraisal_id->toClientList($Page) ?>;
    fmain_pa_employee_ratingsedit.lists.employee_id = <?= $Page->employee_id->toClientList($Page) ?>;
    fmain_pa_employee_ratingsedit.lists.appraisal_status = <?= $Page->appraisal_status->toClientList($Page) ?>;
    fmain_pa_employee_ratingsedit.lists.isactive = <?= $Page->isactive->toClientList($Page) ?>;
    loadjs.done("fmain_pa_employee_ratingsedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fmain_pa_employee_ratingsedit" id="fmain_pa_employee_ratingsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_main_pa_employee_ratings_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_main_pa_employee_ratings_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="main_pa_employee_ratings" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_main_pa_employee_ratings_appraisal_id">
    <select
        ...
        data-select2-id="fmain_pa_employee_ratingsedit_x_appraisal_id"
...
<script>
loadjs.ready("fmain_pa_employee_ratingsedit", function() {
    var options = { name: "x_appraisal_id", selectId: "fmain_pa_employee_ratingsedit_x_appraisal_id" };
    if (fmain_pa_employee_ratingsedit.lists.appraisal_id.lookupOptions.length) {
        options.data = { id: "x_appraisal_id", form: "fmain_pa_employee_ratingsedit" };
    } else {
        options.ajax = { id: "x_appraisal_id", form: "fmain_pa_employee_ratingsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_main_pa_employee_ratings_employee_id">
    <select
        ...
        data-select2-id="fmain_pa_employee_ratingsedit_x_employee_id"
        data-table="main_pa_employee_ratings"
...
<script>
loadjs.ready("fmain_pa_employee_ratingsedit", function() {
    var options = { name: "x_employee_id", selectId: "fmain_pa_employee_ratingsedit_x_employee_id" };
    if (fmain_pa_employee_ratingsedit.lists.employee_id.lookupOptions.length) {
        options.data = { id: "x_employee_id", form: "fmain_pa_employee_ratingsedit" };
    } else {
        options.ajax = { id: "x_employee_id", form: "fmain_pa_employee_ratingsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="form-check d-inline-block">
    <input type="checkbox" class="form-check-input<?= $Page->isactive->isInvalidClass() ?>" data-table="main_pa_employee_ratings" data-field="x_isactive" name="x_isactive[]" id="x_isactive_148492" value="1"<?= ConvertToBool($Page->isactive->CurrentValue) ? " checked" : "" ?><?= $Page->isactive->editAttributes() ?> aria-describedby="x_isactive_help">
